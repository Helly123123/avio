  ...
  static async resetAllLimits() {
    try {
      const [users] = await pool.query(
        "SELECT uuid, subscription FROM users"
      );
      ...
      const withSubscription = users.filter(u => u.subscription === 1).map(u => u.uuid);
      const withoutSubscription = users.filter(u => u.subscription !== 1).map(u => u.uuid);
